...
public class ConsInfSolver {
	...
	public static double wRel;
	
	public static double constrainedInf(List<Problem> testData, SLModel relModel, 
			SLModel pairModel, String dataset) throws Exception {
		...
		for(Problem prob : testData) {
			...
			double ans = ConsInfSolver.getBestStructure(
					prob.ta, prob.quantities, relScores, pairScores, dataset);
	...
	public static double getBestStructure(TextAnnotation ta, List<QuantSpan> quantities, 
			Map<String, Double> relScores, Map<String, Double> pairScores, String dataset) throws Exception {
		...
		for(Pair<List<Node>, Double> state : beam1) {
			...
			if(!isInteger && !dataset.equals("AI2")) continue;
			beam2.add(new Pair<List<Node>, Double>(state.getFirst(), state.getSecond()));
		}
		if(beam2.size() == 0) return beam1.element().getFirst().get(0).getValue();
...
//				if(Math.abs(leaf1.quantIndex - leaf2.quantIndex) < 1.5) continue;
	...
	public static double getIrrelevanceScore(
		...
		return relScore.get(index+"_false")*wRel;
	}
